...
/* GUI-only askpass executable (copied); NULL restores terminal prompting. */
...
/* All family/protocol variants (owned strings, free with TRUE). */
...
gboolean tc_valid_ip(const char *s); /* address or CIDR prefix */
...
/* Owned IFB name for a source interface; FALSE if the interface is missing. */
